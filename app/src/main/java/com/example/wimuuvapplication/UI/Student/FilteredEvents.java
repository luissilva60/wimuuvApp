...
public class FilteredEvents extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listViewFiltered = findViewById(R.id.listEventsFiltered);
        events = new ArrayList<String>();
        JSONObject obj;
        JSONArrayDownloader task = new JSONArrayDownloader();
        JSONArrayDownloader task2 = new JSONArrayDownloader();
        JSONArrayDownloader task3 = new JSONArrayDownloader();
        SPOTID1 = Integer.parseInt(FilterActivity.SPOT_ID);
        TYPEID1 = Integer.parseInt(FilterActivity.TYPE_ID);

        Log.e("SPOT ID",""+SPOTID1);
        Log.e("TYPE ID",""+TYPEID1);
        Toolbar toolbar2 = (Toolbar) findViewById(R.id.toolbar2);
        toolbar2.setTitle("Eventos neste Spot: ");
        setSupportActionBar(toolbar2);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        if (SPOTID1 != 0 && TYPEID1 == 0){
            try {
                objevents = task.execute("https://wimuuv.herokuapp.com/api/events/spot/" + FilterActivity.SPOT_ID).get();
        ...
        } if (TYPEID1 != 0 && SPOTID1 == 0) {
            try {
                objevents = task2.execute("https://wimuuv.herokuapp.com/api/events/type/" + FilterActivity.TYPE_ID).get();
            } catch (InterruptedException e) {
                e.printStackTrace();
            } catch (ExecutionException e) {
                e.printStackTrace();
            ...
            try {
                objevents = task3.execute("https://wimuuv.herokuapp.com/api/events/spot/" + FilterActivity.SPOT_ID + "/type/"+ FilterActivity.TYPE_ID).get();
            } catch (InterruptedException e) {
                e.printStackTrace();
            } catch (ExecutionException e) {
                e.printStackTrace();
    ...
    public void InitializeAdapter() {
        ...
        listViewFiltered.setAdapter(adapterEvents);
        createListViewClickItemEvent(listViewFiltered, events, eventId, eventName, eventDescription, eventDate, eventStartTime, eventEndTime, eventTypeId, eventStateId, eventOrgId, eventSpotId);
    }
